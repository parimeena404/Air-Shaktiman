...
// Flipkart-style interconnected portals switcher
const PORTAL_NAV = [
  { id: 'user', label: 'User', emoji: '👤', path: '/user', color: '#4285F4', bg: '#E8F0FE', role: 'student' as const },
  { id: 'admin', label: 'Admin', emoji: '🛡️', path: '/organization', color: '#EA4335', bg: '#FCE8E6', role: 'admin' as const },
  { id: 'company', label: 'Company', emoji: '🏢', path: '/csr', color: '#FBBC04', bg: '#FEF7E0', textHover: '#E37400', role: 'corporate' as const },
  { id: 'government', label: 'Government', emoji: '🏛️', path: '/government', color: '#34A853', bg: '#E6F4EA', role: 'government' as const },
];
...
export const Header: React.FC = () => {
  ...
  return (
    <div className="sticky top-0 z-30 select-none">
      <header
        className="backdrop-blur-md px-3 sm:px-4 md:px-6 py-2.5 flex items-center justify-between"
        style={{
      ...
      >
        ...
        {/* Center: Flipkart-style Connected Portal Switcher (Desktop & Tablet) */}
        <nav
          className="hidden md:flex items-center gap-1 p-1 rounded-full bg-[#F1F3F4] border border-[#E8EAED]"
          aria-label="Portals"
        >
          {PORTAL_NAV.map((p) => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: isActive ? p.color : '#5F6368',
                  boxShadow: isActive ? '0 1px 4px rgba(60, 64, 67, 0.15)' : 'none',
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative">
            <button
              ...
              style={{
                backgroundColor: showNotifications ? portalStyle.bg : '#F1F3F4',
                color: showNotifications ? portalStyle.color : '#5F6368',
              }}
            >
      ...
      {/* Mobile Sub-Header: Connected Portal Switcher */}
      <div
        className="md:hidden flex items-center gap-1.5 px-3 py-2 overflow-x-auto no-scrollbar"
        style={{
          backgroundColor: '#FFFFFF',
          borderBottom: '1px solid #E8EAED',
        }}
      >
        <span className="text-[10px] font-bold tracking-wider uppercase text-gray-400 mr-1 flex-shrink-0">Portals:</span>
        {PORTAL_NAV.map((p) => {
          const isActive = (pathname || '').startsWith(p.path);
          return (
            <button
              ...
              style={{
                backgroundColor: isActive ? p.bg : '#F8F9FA',
                color: isActive ? p.color : '#5F6368',
                border: `1px solid ${isActive ? p.color + '60' : '#E8EAED'}`,
                boxShadow: isActive ? '0 1px 3px rgba(60, 64, 67, 0.1)' : 'none',
              }}
            >
              <span>{p.emoji}</span>
              <span>{p.label}</span>
